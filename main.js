...
const sortingAlgorithms = document.querySelectorAll(
	"input[name=sorting-method]"
);
	...
	function delegateClickEvent(headerColumns) {
		Status.delegateOnclickEvent({
			...
			func: e => {
				...
				const selectedSortingMethod = document.querySelector(
					"input[name=sorting-method]:checked"
				).value;
...
const updateBtnDisabledObserverConfig = {
	...
	withClass: () => {
		Status.dynamicElementObserver(
			`thead th`,
			headerColumns => {
				Status.removeElementOnclickEvent([
					select,
					sortingMethodGroup,
					...headerColumns,
				]);
...
const parseHandler = cb => {
	...
	reader.onload = async function (e) {
		...
		displayMethod(
			dataRecorder.parsedFileContentHeader,
			dataRecorder.parsedFileContentBody
		);
		cb(dataRecorder);
	};
};
...
const onUpdate = dataRecorder => {
	...
	if (!document.querySelector("input[name=sorting-method]:checked")) {
		Status.setStatusText(
			"Please select a sorting method from the settings first"
		);
		return;
	}

	const algorithmName = document.querySelector(
		"input[name=sorting-method]:checked"
	).value;
	...
	sortWorker.onmessage = function (message) {
		...
		dataRecorder.fileContentRecords.forEach(record => {
			record = Object.assign(recordType[fileType](), record);
		});
		...
		downloadBtn.onclick = () => {
			downloadCSVFile(
				dataRecorder.sortedFileContent.join("\n"),
